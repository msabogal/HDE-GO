...
int background_functions(
  ...
  /* fluid with w(a) and constant cs2 */   /*Aqui*/
...
int background_w_fld(                                 /* Aqui es donde se define el fluido de la ecuacion de estado*/
                     struct background * pba,
                     double a,
                     double * w_fld,
  ...
  /** - first, define the function w(a) */  /*Aqui se colocan los casos de w */
  ...
  case EDE:
    ...
    *w_fld = - dOmega_ede_over_da*a/Omega_ede/3./(1.-Omega_ede)+a_eq/3./(a+a_eq);
    break;
  case GO:   /*Aqui caso GO */
    if (pba->has_GO == _TRUE_ ){
      Omega_r = pba->Omega0_g + pba->Omega0_ur;
      Omega_m = pba->Omega0_b + pba->Omega0_cdm;

    *w_fld = ( (-2.+ 2.*pba->alpha_GO-3.*pba->beta_GO)*( pow(a,4. + 2./pba->beta_GO)*( (-1.+pba->alpha_GO-2.*pba->beta_GO)*(-2.+2.*pba->alpha_GO-3.*pba->beta_GO + 2.*Omega_m)
      + (-2.+2.*pba->alpha_GO-3.*pba->beta_GO)*Omega_r ) - pow(a,2.*pba->alpha_GO /pba->beta_GO)*(pba->alpha_GO-2.*pba->beta_GO)*pba->beta_GO*Omega_r ) )
      /( 3.*pba->beta_GO*( pow(a,4. + 2./pba->beta_GO)*( (-1.+pba->alpha_GO-2.*pba->beta_GO)*(-2.+2.*pba->alpha_GO-3.*pba->beta_GO + 2.*Omega_m)
  ...
  *dw_over_da_fld = ( (-2.+2.*pba->alpha_GO-3.*pba->beta_GO)*( pow(a,4.*pba->alpha_GO/pba->beta_GO)*pow(pba->beta_GO,2.)*(-1.*pba->alpha_GO + 2.*pba->beta_GO)*(1.-pba->alpha_GO+2.*pba->beta_GO)
    *(-2.*pba->alpha_GO+3.*pba->beta_GO)*Omega_m*Omega_r - pow(a,4.+ 2.*(1+pba->alpha_GO)/pba->beta_GO)*(1.-pba->alpha_GO+2.*pba->beta_GO)*(-2.*pba->alpha_GO+3.*pba->beta_GO)
  ...
  Omega_m = pba->Omega0_b + pba->Omega0_cdm;
    ...
    *integral_fld = (4. + 2.*(pba->alpha_GO / pba->beta_GO) )*log(1/a)
    + log( (pow(a, 4. + 2./pba->beta_GO)*( (-1.+pba->alpha_GO -2.*pba->beta_GO)*(-2.+2.*pba->alpha_GO-3.*pba->beta_GO + 2.*Omega_m)
    + (-2.+2.*pba->alpha_GO-3.*pba->beta_GO)*Omega_r)
    - pow(a,1.+ 2.*pba->alpha_GO/pba->beta_GO)*(2.*pba->alpha_GO -3.*pba->beta_GO)*(-1+pba->alpha_GO-2.*pba->beta_GO)*Omega_m
    - pow(a,2.*pba->alpha_GO/pba->beta_GO)*(-2.+2.*pba->alpha_GO-3.*pba->beta_GO)*(pba->alpha_GO-2.*pba->beta_GO)*Omega_r)
...
/*------------------------------------------------------ */
  pba->has_GO = _FALSE_;  /* Aqui se indica deafult si tiene o no GO*/
/*------------------------------------------------------ */
  ...
  /*------------------------------------------------------ */
  if ( (pba->alpha_GO != 0.) && (pba->beta_GO !=0.))  /* Aqui coloqua si tiene GO*/
    pba->has_GO = _TRUE_;
...
int background_solve(
  ...
  switch (ppr->background_evolver) {     /* Revisar caso para elegir el evolver*/
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_) {  /* Aqui*/
    ...
    if ( integral_fld != integral_fld) {
      class_stop(pba->error_message,"Energy condition violated rho_fld < 0");
    }
